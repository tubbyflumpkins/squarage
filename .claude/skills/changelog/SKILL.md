...
- Shipped to production 2026-09-21 and exercised against Shopify the same day (draft created with discounts off, reprice, fixed and free delivery lines, revoke deletes the draft). `LABS_API_URL` is set for Production and Preview.
- **Layout pass (2026-09-21, Dylan)**: the centre column is the viewer alone (the "Your Custom …" title bar is gone); the right column opens with its own bar, "[Warped Shelf / Warped Corner Shelf / Warped Console] prepared for [name]" as the page's one `<h1>` with the customer's email under it (labs now sends `customerEmail`), then the notes; Layout gained Shelf Height and Shelf Width — labs' Measurements numbers, centre to centre (`shelfSpacing`, and `(width - 2·columnOffset) / (columns - 1)`; no width on a corner, whose columns fan across two walls). On mobile the title bar leads the scrolling column as a `<p>`.
